...
function AddEmployee() {
  ...
  // Modify your submitHandle function to include the imageId
function submitHandle(e) {
  e.preventDefault();
  
  if (!name || !phone || !dateOfBirth || !dateOfJoining || !department || 
      !employmentStatus || !gender || !profileImage || !address.state || 
  ...
  axios
    .post(`${server_url}/create`, {
      ...
      gender, 
      address,
      imageId: profileImage  // Send the image ID we got from upload
    })
    .then((result) => {
      console.log(result);
      navigate('/');
  ...
  return (
    <div className='add'>
      ...
      <div>
        {/* Date of Birth Field - Standalone */}
        <div>
          <label htmlFor="dateOfBirth">Date of Birth</label> 
          <br/>
          <input
  ...
  {/* Date of Joining Field - Standalone */}
  <div>
    <label htmlFor="dateOfJoining">Date of Joining</label>
    <br/>
    <input
      ...
      onChange={(e) => {
        ...
        if (dateOfBirth && selectedDate < dateOfBirth) {
          alert("Joining date cannot be before birth date");
          e.target.value = '';
